...
export default function UserProfile() {
  ...
  //     fetch(`http://192.168.1.13:8081/info/userinfo/${userId}`)
  //       .then((res) => res.json())
  //       .then((data) => {
  //         setUserInfo(data);
  //         setIsLoading(false);
  ...
  useEffect(() => {
    if (isInitialRender.current) {
      ...
      const apiUrl = `http://192.168.1.13:8081/info/userinfo/${userId}`;
  ...
  useEffect(() => {
    ...
    const apiUrl = "http://192.168.1.13:8081/admin/module";
    const token = getTokenFromLocalStorage();
    const headers = {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`, 
