...
export default function SetModuleMaterials() {
  ...
  const fetchData = (buttonType) => {
    const apiUrl = `http://192.168.1.2:8081/${buttonType}/${selectedCategory}`;
    fetch(apiUrl)
      .then((res) => res.json())
      .then((data) => {
        setApiData(data);
    ...
    if (selectedButton === "documents" || selectedButton === "videos") {
      const apiUrl =
        selectedButton === "documents"
          ? "http://192.168.1.2:8081/documents"
          : "http://192.168.1.2:8081/videos";
  ...
  const handleDelete = (itemId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const apiUrl =
          selectedButton === "documents"
            ? `http://192.168.1.2:8081/documents/${itemId}`
            : `http://192.168.1.2:8081/videos/${itemId}`;
