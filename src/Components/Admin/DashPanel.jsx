...
export default function DashPanel() {
  ...
  // useEffect(() => {
  //   fetch("http://192.168.1.3:8081/dashboard/admin")
  //     .then((res) => res.json())
  //     .then((data) => {
  //       setDashData(data);
  //     });
  ...
  useEffect(() => {
    ...
    if (token) {
      axios
        .get("http://192.168.1.3:8081/dashboard/admin", {
          headers: {
  ...
  const handleVisibility = (modId) => {
    ...
    fetch(`http://192.168.1.3:8081/admin/module/visibility/${modId}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  const handleModuleDelete = (modId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        fetch(`http://192.168.1.3:8081/admin/module/${modId}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
          },
