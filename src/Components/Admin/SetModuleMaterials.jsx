...
export default function SetModuleMaterials() {
  ...
  const fetchData = (buttonType) => {
    const apiUrl = `http://192.168.1.3:8081/${buttonType}/${selectedCategory}`;
    fetch(apiUrl, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  const handleDelete = (itemId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const apiUrl =
          selectedButton === "documents"
            ? `http://192.168.1.3:8081/documents/${itemId}`
            : `http://192.168.1.3:8081/videos/${itemId}`;
            ...
            <div className="flex justify-between">
              <div className="w-full">
                ...
                <form action="">
                  <div className="mt-8">
                    <div>
                      <p className="mb-1 font-bold ms-1">Bulk upload</p>
                    </div>
